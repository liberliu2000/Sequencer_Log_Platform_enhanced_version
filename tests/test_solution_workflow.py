...
def _payload(seed: str) -> dict:
    return {
        ...
        "submodule": "XYZ platform",
        "message": f"Axis timeout while homing {seed}",
        "normalized_signature": f"sig_{seed}",
        "trigger_scenario": "设备启动后执行回零流程，平台在 Z 轴回原点阶段超时",
        "impact_scope": "当前批次无法继续运行",
        "root_cause_analysis": "Z 轴回零等待条件过严，传感器抖动导致状态机无法稳定收敛。",
        "verified_solution": "放宽去抖时间并补充传感器异常重试，现场复现后问题消失。",
        "workaround": "重启后单独执行一次回零流程。",
        "owner_department": "motion_control",
        "submitter": "pytest",
        "source": "unit_test",
        "task_clusters": ["流程执行"],
        "tags": ["motion", "timeout"],
        "message_keywords": ["axis", "timeout", "homing"],
        "reusable": True,
    }
...
def test_solution_repository_create_search_export():
    ...
    try:
        ...
        assert created["error_code_prefix"] == "MC"
        assert created["task_clusters"]
        assert created["message_keywords"]
        rows = service.list_records(normalized_signature=f"sig_{seed}", limit=10, viewer_is_reviewer=True)
